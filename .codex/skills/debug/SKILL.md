...
name: debug
description:
  Investigate stuck runs and execution failures by tracing sardine-run and Codex
...
- Primary runtime log: `log/sardine-run.log`
  - Default comes from `SardineRun.LogFile` (`log/sardine-run.log`).
  - Includes orchestrator, agent runner, and Codex app-server lifecycle logs.
- Rotated runtime logs: `log/sardine-run.log*`
...
rg -n "session_id=<thread>-<turn>" log/sardine-run.log*

# 5) Focus on stuck/retry signals
rg -n "Issue stalled|scheduling retry|turn_timeout|turn_failed|Codex session failed|Codex session ended with error" log/sardine-run.log*
```
...
In sardine-run, Codex session diagnostics are emitted into `log/sardine-run.log` and
...
- Check rotated logs (`log/sardine-run.log*`) before concluding data is missing.
