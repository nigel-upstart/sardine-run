defmodule SardineRun.Orchestrator do
  @moduledoc """
  ...
  defp do_maybe_dispatch(%State{} = state) do
    with :ok <- Config.validate!(),
      ...
      {:error, :missing_state_repo} ->
        Logger.error("Traffic Control state repo missing in WORKFLOW.md (set tracker.state_repo or $TRAFFIC_CONTROL_STATE_REPO)")
        state

      {:error, :state_repo_not_configured} ->
        Logger.error("Traffic Control state repo not configured")
        state
  ...
  defp terminate_task(pid) when is_pid(pid) do
    case Task.Supervisor.terminate_child(SardineRun.TaskSupervisor, pid) do
      :ok ->
        :ok
