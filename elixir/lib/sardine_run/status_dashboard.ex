defmodule SardineRun.StatusDashboard do
  @moduledoc """
  ...
  alias SardineRun.{Config, HttpServer}
  ...
  def render_offline_status do
    content =
      [
        colorize("╭─ 🐟 SARDINE RUN STATUS", @ansi_bold),
  ...
  defp format_project_link_lines do
    project_part =
      case Config.settings!().tracker.state_repo do
        state_repo when is_binary(state_repo) and state_repo != "" ->
          colorize(state_repo, @ansi_cyan)

        _ ->
          colorize("n/a", @ansi_gray)
      end

    project_line = colorize("│ State repo: ", @ansi_bold) <> project_part
