defmodule SardineRun.HttpServer do
  @moduledoc """
  Compatibility facade that starts the Phoenix observability endpoint when enabled.
  """

  alias SardineRun.{Config, Orchestrator}
  alias SardineRunWeb.Endpoint
    ...
    case Keyword.get(opts, :port, Config.server_port()) do
      port when is_integer(port) and port >= 0 ->
        ...
        with {:ok, ip} <- parse_host(host) do
          ...
          Application.put_env(:sardine_run, Endpoint, endpoint_config)
          Endpoint.start_link()
        end
