...
This document explains how Codex reports token usage through the app-server protocol and how Sardine Run should account for it.
...
Sardine Run sees these events wrapped inside the app-server message stream.
...
For Sardine Run, `model_context_window` should be displayed or logged separately from spend.

## Recommended Accounting Strategy For Sardine Run
...
## What Sardine Run Should And Should Not Do
...
- Expect one thread to span multiple turns when Sardine Run reuses a live Codex thread.
...
## Practical Interpretation For Sardine Run Logs
...
That is a strong signal for Sardine Run:

- use absolute totals as the main accounting surface
- ignore last/delta values for totals

## Recommended Sardine Run Documentation Contract

If Sardine Run documents token reporting externally, the contract should be:

- Live token totals come from Codex thread-scoped cumulative usage.
- Incremental usage may also be emitted, but Sardine Run does not use it for totals.
